...
import { assertHTML, evaluate, focusEditor, html, initialize, LEGACY_EVENTS, test } from '../utils/index.mjs';
...
test.describe('Events', () => {
  ...
  test('Autocapitalization (MacOS specific)', async ({ page, isPlainText }) => {
    ...
    await evaluate(page, () => {
      ...
      function singleRangeFn(startContainer, startOffset, endContainer, endOffset) {
        return () => [
          new StaticRange({
            endContainer,
            endOffset,
      ...
      characterBeforeInputEvent.getTargetRanges = singleRangeFn(textNode, 1, textNode, 1);
      ...
      replacementBeforeInputEvent.getTargetRanges = singleRangeFn(textNode, 0, textNode, 1);
  ...
  test('Add period with double-space after emoji (MacOS specific) #3953', async ({ page, isPlainText }) => {
    if (LEGACY_EVENTS) {
      return;
    }
    await focusEditor(page);
    ...
    await evaluate(page, () => {
      ...
      characterBeforeInputEvent.getTargetRanges = singleRangeFn(lastSpanTextNode, 0, lastSpanTextNode, 1);
