...
test.describe('File', () => {
  ...
  test(`Can import/export`, async ({ page, isPlainText }) => {
    ...
    await insertUploadImage(page, ['packages/lexical-playground/src/images/yellow-flower-small.jpg']);

    await waitForSelector(page, '.editor-image img');

    const expectedHtml = html`
      <p class="PlaygroundEditorTheme__paragraph PlaygroundEditorTheme__ltr" dir="ltr">
        <strong class="PlaygroundEditorTheme__textBold" data-lexical-text="true"> Hello </strong>
    ...
    const [download] = await Promise.all([page.waitForEvent('download'), click(page, '.action-button.export')]);
